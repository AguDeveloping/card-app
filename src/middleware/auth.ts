...
export const generateToken = (userId: string): string => {
  const payload = {
    sub: userId,
    // iat and 
    // exp will be set automatically by jwt.sign
    type: 'access' // Token type
  };
  const token = jwt.sign(payload, JWT_SECRET, {
    expiresIn: '24h' // This automatically adds 'exp' and 'iat'
  });
  return token;
};
...
export const requireAuth = (req: any, res: any, next: any) => {
  ...
  // Preliminary token decode for logging
  if (authHeader) {
  ...
  // Use passport to authenticate the JWT token
